...
function WeatherCard() {
    ...
    const weatherData = [
        ...
        { time: "18시", temp: "17℃", icon: weatherTest},
        { time: "19시", temp: "17℃", icon: weatherTest},
        { time: "20시", temp: "16℃", icon: weatherTest},
        { time: "21시", temp: "17℃", icon: weatherTest},
        { time: "22시", temp: "16℃", icon: weatherTest},
        { time: "23시", temp: "16℃", icon: weatherTest},
    ]
...
">
  ...
  <div>
    ...
    <div className="flex items-end gap-2 mt-1">
      ...
      <span className="text-[28px] font-bold text-brand-700">
        32℃
      </span>
    ...
    🐾
      ...
      <div className="flex justify-center mt-4 overflow-x-auto">
        {weatherData.map((weather, index) => (
            <div key={index} className={`flex flex-col items-center min-w-[45px] px-4 ${index !== weatherData.length-1 ? "border-r border-gray-200" : ""}`}>
                {/* 시간 */}
                <span className='text-[14px] text-gray-500'>
                    {weather.time}
                </span>
                {/* 아이콘 */}
                <img src={weather.icon} alt="날씨아이콘" className="w-7 h-7 object-cover mt-3 my-1"/>
                {/* 온도 */}
